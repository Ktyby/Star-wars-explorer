...
export function* loadPeopleData() {
	try {
		const responseData = yield call(() =>
			axios.get(`${API_URL}/people/?page=1`).then((response) => response.data)
		);
		const data = responseData.results;
		const nextPage = responseData.next;
		yield put(loadPeopleDataSuccess(data, nextPage));
	} catch (error) {
...
export function* loadPlanetsData() {
	try {
		const responseData = yield call(() =>
			axios.get(`${API_URL}/planets/?page=1`).then((response) => response.data)
		);
		const data = responseData.results;
		const nextPage = responseData.next;
		yield put(loadPlanetsDataSuccess(data, nextPage));
	} catch (error) {
...
export function* loadStarshipsData() {
	try {
		const responseData = yield call(() =>
			axios.get(`${API_URL}/starships/?page=1`).then((response) => response.data)
		);
		const data = responseData.results;
		const nextPage = responseData.next;
		yield put(loadStarshipsDataSuccess(data, nextPage));
	} catch (error) {
...
export function* loadMorePeopleData(action) {
	try {
		const responseData = yield call(() => axios.get(action.page).then((response) => response.data));
		const data = responseData.results;
		const nextPage = responseData.next;
...
export function* loadPeopleTilesData(action) {
	try {
		...
		const filmsRequests = peopleData.films.map((element) =>
			axios.get(element).then((response) => response.data)
		);
		...
		peopleData.homeworld = yield call(() =>
			axios.get(peopleData.homeworld).then((response) => response.data)
		);
